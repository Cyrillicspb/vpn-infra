...
ssh -i "!SSH_KEY!" -o "StrictHostKeyChecking=accept-new" -p !SSH_PORT! !SERVER_USER!@!SERVER_IP! "sudo mkdir -p /opt/vpn && sudo tar xzf /tmp/vpn-infra.tar.gz -C /opt/vpn --no-same-permissions --no-same-owner --overwrite --touch; rm -f /tmp/vpn-infra.tar.gz"
if !errorlevel! neq 0 (
...
if !PY_CMP! GEQ 310 (
    ...
    if !errorlevel! equ 0 (
        ...
        ssh -p !SSH_PORT! -i "!SSH_KEY!" -o "StrictHostKeyChecking=accept-new" -o "BatchMode=yes" !SERVER_USER!@!SERVER_IP! "sudo apt-get install -y -qq python3-pip python3-venv 2>/dev/null; sudo pip3 install textual --break-system-packages --ignore-installed --quiet" >nul 2>&1
