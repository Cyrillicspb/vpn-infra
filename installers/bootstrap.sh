...
if ! python3 -c 'import textual' 2>/dev/null; then
    ...
    if [[ -d "$WHEELS_DIR" ]] && ls "$WHEELS_DIR"/*.whl &>/dev/null 2>&1; then
        ...
        err "Локальный wheel bundle для installer GUI отсутствует. Основной режим установки требует bundled TUI dependencies."
        err "Используйте полный release bundle или перейдите к console fallback через setup.sh."
        exit 1
    fi
fi
