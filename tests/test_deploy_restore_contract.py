...
class DeployRestoreContractTests(unittest.TestCase):
    ...
    def test_watchdog_uses_source_tree_for_telegram_repo_sync(self):
        ...
        self.assertIn('NFTABLES_GENERATOR_SOURCE_FILE = Path("/opt/vpn/home/scripts/generate-nftables.sh")', watchdog)
        ...
        self.assertIn('f"Unit: `{plugin.dataplane_unit_name(tun_name)}`\\n"', watchdog)
        self.assertIn('f"Meta: `{process_meta}`\\n"', watchdog)
        self.assertIn("dnsmasq_expected_drift_reason", watchdog)
        self.assertIn("dnsmasq_expected_drift_until_ts", watchdog)
        self.assertIn("_remember_expected_dnsmasq_drift", watchdog)
        self.assertIn("_consume_expected_dnsmasq_drift", watchdog)
        self.assertIn('f"{_reason_detail_suffix(reason)}"', watchdog)
        ...
        self.assertIn("def start_tun2socks_service", base_plugin)
        self.assertIn("def stop_tun2socks_service", base_plugin)
    ...
    def test_admin_bot_texts_match_deploy_status_contract(self):
        ...
        self.assertIn('adm:cl_dev_direct:', admin_handler)
        self.assertIn('adm:cl_dev_directb:', admin_handler)
        self.assertIn('adm:cl_dev_directs:', admin_handler)
        ...
        self.assertIn("_send_admin_direct_backend_export", admin_handler)
        self.assertIn("Recovery-пакет", admin_handler)
        self.assertIn("artifact_mode", admin_handler)
        self.assertIn("import_hint", admin_handler)
        ...
        self.assertIn("Direct backend", keyboards)
        ...
        self.assertIn('"/config/direct-export"', watchdog_client)
        ...
        self.assertIn('@app.post("/config/direct-export")', (ROOT / "home" / "watchdog" / "watchdog.py").read_text(encoding="utf-8"))
        watchdog_source = (ROOT / "home" / "watchdog" / "watchdog.py").read_text(encoding="utf-8")
        self.assertIn('artifact_mode = "uri_only"', watchdog_source)
        self.assertIn('artifact_mode = "file_and_uri"', watchdog_source)
        self.assertIn("runtime JSON шаблон", watchdog_source)
        client_handler = (ROOT / "home" / "telegram-bot" / "handlers" / "client.py").read_text(encoding="utf-8")
        self.assertIn("get_device_for_client", (ROOT / "home" / "telegram-bot" / "database.py").read_text(encoding="utf-8"))
        self.assertIn("_get_owned_device", client_handler)
        self.assertIn("artifact_mode", client_handler)
        self.assertIn("import_hint", client_handler)
        self.assertIn('device = await _get_owned_device(db, chat_id, device_id)', client_handler)
        self.assertNotIn('device = await db.get_device_by_id(device_id)', client_handler)
    ...
    def test_graph_panel_contract_matches_dashboard_json(self):
        ...
        self.assertIn('"tun2socks-stack@.service"', installer)
        ...
        self.assertIn("_run_stack_runtime_probes_for_stack", watchdog_source)
        self.assertIn("_stack_cooldown_reason", watchdog_source)
        ...
        tun2socks_stack_unit = (ROOT / "home" / "systemd" / "tun2socks-stack@.service").read_text(encoding="utf-8")
        self.assertIn("Description=tun2socks dataplane for VPN stack (%i)", tun2socks_stack_unit)
        self.assertIn("EnvironmentFile=-/run/tun2socks-stack/%i.env", tun2socks_stack_unit)
        self.assertIn("Restart=on-failure", tun2socks_stack_unit)
        self.assertIn("SyslogIdentifier=tun2socks-stack@%i", tun2socks_stack_unit)
        cloudflare_client = (ROOT / "home" / "watchdog" / "plugins" / "cloudflare-cdn" / "client.py").read_text(encoding="utf-8")
        hysteria_client = (ROOT / "home" / "watchdog" / "plugins" / "hysteria2" / "client.py").read_text(encoding="utf-8")
        nft_generator = (ROOT / "home" / "scripts" / "generate-nftables.sh").read_text(encoding="utf-8")
        self.assertIn("start_tun2socks_service", cloudflare_client)
        self.assertIn("stop_tun2socks_service", cloudflare_client)
        self.assertIn("start_tun2socks_service", hysteria_client)
        self.assertIn("stop_tun2socks_service", hysteria_client)
        self.assertIn('ROUTER_LAN_IP="${ROUTER_LAN_IP:-${LAN_GATEWAY_IP:-${GATEWAY_IP:-}}}"', nft_generator)
        self.assertIn('tcp dport { 80, 443 } dnat to', nft_generator)
