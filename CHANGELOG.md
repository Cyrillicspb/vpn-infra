...
## [v0.3.2.1] — 2026-03-22 — deploy.sh: git fetch через vps_tmux_exec

### Исправления

- **deploy.sh**: `git fetch` на VPS-зеркале переведён с `vps_exec` на `vps_tmux_exec` (timeout 60s). Операция 10-30 сек, единственная незащищённая SSH-операция при эксплуатации.

---

## [v0.3.2.0] — 2026-03-22 — Исправление tmux: запуск из установщика, не из скрипта
...
## [v0.3.1.9] — 2026-03-22 — vps_exec_long: авто-реконнект при обрыве SSH

### Улучшения

- **install-vps.sh**: `vps_exec_long` теперь автоматически переподключается при обрыве SSH и продолжает стримить вывод. Команда на VPS продолжает работать, retry каждые 10 сек.
...
- **install-home.sh** (шаг 11): tmux авто-старт добавляется в `.bashrc` для `sysadmin` и `root`. При каждом SSH-подключении автоматически присоединяется к сессии `main` или создаёт новую. Защищает все долгие операции (deploy, backup, диагностика) от обрывов SSH.

---

## [v0.3.1.6] — 2026-03-22 — Tmux: авто-переход после шага 10

### Улучшения

- **install-home.sh**: добавлен `tmux` в системные пакеты (шаг 10).
- **install-home.sh**: после шага 10 скрипт автоматически перезапускается внутри tmux-сессии `vpn-install`. Шаги 1–10 быстрые и без обрывов; шаги 11+ (docker build, nmap и др.) теперь защищены от разрыва SSH. При повторном подключении: `tmux attach -t vpn-install`.

---

## [v0.3.1.5] — 2026-03-22 — Добавить ncat в системные пакеты (autossh-tier2)

### Исправления

- **install-home.sh** (шаг 10): добавлен пакет `ncat` в список системных пакетов. `autossh-tier2` использует `ncat` как ProxyCommand для SOCKS5 — без него туннель не поднимался (`ncat: not found`), DNS через VPS не работал.

---

## [v0.3.1.4] — 2026-03-22 — Исправление метки "WG туннель" → "SSH tier-2 туннель"

### Исправления

- **post-install-check.sh**: надпись "tier-2 WG туннель не поднят" исправлена на "SSH tier-2 туннель не поднят (autossh-tier2)". WireGuard tier-2 упразднён, туннель к VPS работает через autossh SSH.
...
- **release.yml**: версии hysteria2 и tun2socks в шаге "Скачать бинарники" теперь определяются автоматически через GitHub API вместо захардкоженных значений. Фоллбэк на последнюю известную версию если API недоступен. Предотвращает 404 при удалении старых релизов из upstream.

---

## [v0.3.1.0] — 2026-03-22 — Обновление hysteria2 до v2.7.1

### Исправления

- **install-home.sh** + **release.yml**: hysteria2 обновлён с v2.5.1 до v2.7.1. Старая версия удалена с GitHub → URL возвращал 404 → установка падала с `die()`.
- **install-home.sh** (шаг 17): добавлен GitHub API auto-detection для hysteria2 с фоллбэком на hardcoded версию. При следующей смене версии установщик автоматически подхватит актуальный релиз.
...
- **release.yml**: исправлен шаг распаковки tun2socks — использован флаг `-d` для unzip и `find` для поиска бинарника вместо `cd /tmp && mv`.
