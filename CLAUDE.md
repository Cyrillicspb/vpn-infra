...
## СОСУЩЕСТВОВАНИЕ С ДРУГИМИ ПРОЕКТАМИ НА СЕРВЕРЕ

Другие проекты (Jellyfin, Nextcloud, Home Assistant и т.п.) можно запускать на том же сервере при соблюдении правил:

**Занятые ресурсы — не использовать:**
- Порты: UDP 51820 (AWG), UDP 51821 (WG), TCP/UDP 53 на 127.0.0.1 (dnsmasq), TCP 8080 (watchdog), TCP 1080/1081 (xray SOCKS5)
- nftables: не делать `nft flush ruleset`, не перезаписывать `/etc/nftables.conf` — сломает kill switch
- Routing tables: 100 и 200 зарезервированы для policy routing
- Docker-сеть: не использовать `172.20.0.0/24` (vpn-net)

**Несовместимые проекты:** Pi-hole/AdGuard Home на порту 53, другой VPN-сервер, проекты управляющие nftables/iptables глобально.

**Подробности:** [docs/FAQ.md — Сосуществование с другими проектами](docs/FAQ.md#сосуществование-с-другими-проектами)
