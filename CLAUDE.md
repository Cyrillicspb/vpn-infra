...
- [ ] Удобный GUI-установщик:
  ...
  **TUI-установщик (Textual, 9 экранов):**
  - Неизменяемый layout: header (шаг N/7) + content + footer (Назад / Помощь / Далее)
  - Экраны: Приветствие → Подключение к серверу → Автодетект сети → VPS → Telegram → Опции → Review → Установка → Завершение
  ...
  - Финальный экран: результаты smoke-тестов + ручные шаги (port forwarding) + SSH-команда
