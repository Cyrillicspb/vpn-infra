...
Трафик между домашним сервером и VPS маскируется одним из 4 протоколов:

| Метод | Как обходит DPI |
|---|---|
| **AmneziaWG** (клиент → сервер) | Обфускация заголовков WireGuard: случайный junk в начале пакета, нестандартные поля — ТСПУ не распознаёт как WireGuard |
| **VLESS+XHTTP+REALITY** | Трафик неотличим от обычного HTTPS к легитимному домену (microsoft.com или cdn.jsdelivr.net). REALITY — TLS-маскировка под реальный сертификат цели |
| **Cloudflare CDN** | Трафик идёт через глобальную CDN-сеть Cloudflare — блокировка невозможна без блокировки всего Cloudflare |
| **Hysteria2 + Salamander** | QUIC-трафик с obfuscation: выглядит как случайный UDP |
