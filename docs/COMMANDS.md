...
- Меню `Клиенты → <клиент> → Устройства клиента`:
  - добавить устройство от имени клиента;
  - получить recovery-конфиг устройства (`.conf` + QR) для пересылки по альтернативному каналу;
  - пересобрать и заново выдать recovery-конфиг;
  - удалить устройство клиента.
...
- Если пользователь ещё не зарегистрирован и не может открыть Telegram, используйте bootstrap invite через `/invite`.
- Если пользователь уже зарегистрирован, используйте меню `Клиенты → <клиент> → Устройства клиента` для recovery:
  - добавить новое устройство без self-service запроса;
  - получить или обновить конфиг конкретного устройства;
  - переслать `.conf` и QR пользователю через альтернативный канал.
