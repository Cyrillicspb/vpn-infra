...
/migrate_vps <IP>                      — перенести конфиги на новый VPS
/migrate_vps <IP> --from-backup        — восстановить ключи и конфиги из бэкапа на новый VPS
```
...
```
/rotate_keys           — ротация WireGuard ключей всех пиров; рассылает новые конфиги клиентам
...
```
/dpi                          — статус zapret: включён/выключен, активные пресеты
/dpi on                       — включить DPI bypass глобально
/dpi off                      — выключить DPI bypass
...
Zapret (nfqws) — параллельный механизм DPI-десинхронизации для SNI-throttling. Не заменяет стеки — работает вместе с ними. Трафик к доменам в `dpi_direct` идёт напрямую через eth0 с nfqws-обработкой TCP-пакетов.
